...
from .utils import get_posts_from_tracked_accounts, get_validated_posts, get_auth_session, take_part_in_the_draw


async def add_all_new_posts_to_database():
    from settings import NUMBER_OF_SCANNED_POST_FROM_ACCOUNT as posts_count

    posts = await get_posts_from_tracked_accounts(posts_count)
